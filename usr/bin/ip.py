...
def main():
    ...
    with Connection(connection_url) as connection:
        ...
        try:
            print_header("Welcomback Boskuuuuu", "Boychongzen")
            
            
            wan_ip_address, device_name = fetch_wan_info(client)
            print_result("Modem Anda", device_name)
            print_result("IP Lama", wan_ip_address)
            
            
            print("Process Ganti IP Boskuuu...")
            initiate_ip_change(client)
            
            time.sleep(5)  
            
            print("Tunggu Ganti IP Boskuuu...")
            wan_ip_address_after_plmn, _ = fetch_wan_info(client)
            print_result("IP Baru", wan_ip_address_after_plmn)
            
            print_success("IP Baru Sukses Boskuuu.....")
